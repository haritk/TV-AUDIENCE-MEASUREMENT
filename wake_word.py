...
keyword_file_path = os.path.join(os.path.dirname(__file__), './hi_barc_linux.ppn')
machine = 'x86_64'
library_path = os.path.join(os.path.dirname(__file__), './lib/linux/%s/libpv_porcupine.so' % machine)
    ...
    try:
        ...
        while True:
            ...
            if (end-start)<2:
                ...
                print("Entering")
                if done == 1:
                    recorded_audio = np.concatenate(record, axis=0).astype(np.int16)
